<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>پروژه کارشناسی - رضا احمدی</title>
  <style>
    body {
      font-family: "Tahoma", sans-serif;
      background-color: #f4f4f9;
      color: #333;
      line-height: 1.8;
      margin: 0;
      padding: 2rem;
      direction: rtl;
    }
    h1, h2, h3 {
      color: #2c3e50;
    }
    .container {
      background: white;
      padding: 2rem;
      border-radius: 12px;
      box-shadow: 0 2px 8px rgba(0,0,0,0.1);
      max-width: 900px;
      margin: auto;
    }
    img {
      max-width: 100%;
      border-radius: 8px;
      margin: 1rem 0;
    }
    code {
      background: #eef;
      padding: 0.2rem 0.4rem;
      border-radius: 4px;
      font-family: monospace;
    }
    .filestructure {
      background: #f9f9f9;
      padding: 1rem;
      border-radius: 8px;
      font-family: monospace;
      white-space: pre-line;
    }
    .footer {
      margin-top: 2rem;
      font-size: 0.9rem;
      color: #666;
    }
    a {
      color: #0077cc;
      text-decoration: none;
    }
    a:hover {
      text-decoration: underline;
    }
  </style>
</head>
<body>
  <div class="container">
    <h1>🎓 پروژه کارشناسی: سیستم مکانیکی با دو استپ موتور و یک سروو موتور</h1>
    
    <p>
      این پروژه، پروژه کارشناسی من در رشته مهندسی برق گرایش الکترونیک دیجیتال در دانشگاه افرش است. در این پروژه یک سامانه مکانیکی طراحی و پیاده‌سازی شده است که با استفاده از <strong>دو موتور پله‌ای</strong> و <strong>یک سروو موتور</strong> کنترل می‌شود.
    </p>

    <h2>🛠️ اجزای اصلی پروژه</h2>
    <ul>
      <li>دو عدد استپ موتور برای حرکت دقیق در دو محور</li>
      <li>یک سروو موتور برای اعمال حرکت زاویه‌ای</li>
      <li>برد آردوینو به عنوان کنترل‌کننده مرکزی</li>
      <li>درایور موتورها (A4988 یا ULN2003)</li>
      <li>منبع تغذیه مناسب برای موتورها</li>
    </ul>

    <h2>⚙️ عملکرد پروژه</h2>
    <p>
      سیستم قادر است با استفاده از استپ موتورهای محور X و Y، موقعیت جسم مورد نظر را تغییر دهد و با کمک سروو موتور، عملیاتی مانند چرخش یا گرفتن را انجام دهد. کنترل کل سیستم توسط آردوینو انجام می‌شود.
    </p>

    <h2>🖼️ تصاویر پروژه</h2>
    <h3>🔌 شماتیک مدار</h3>
    <img src="./Circuit%20new.png" alt="شماتیک مدار">
    <h3>📦 تصویر نهایی پروژه مونتاژ شده</h3>
    <img src="./final.jpg" alt="پروژه نهایی">

    <h2>📁 ساختار فایل‌ها</h2>
    <div class="filestructure">
├── Circuit new.png         # تصویر شماتیک مدار<br>
├── final.jpg               # عکس پروژه نهایی<br>
├── code/                   # پوشه شامل کدهای آردوینو<br>
│   └── main.ino<br>
├── README.html             # فایل معرفی پروژه
    </div>

    <h2>📦 وابستگی‌ها</h2>
    <ul>
      <li>نرم‌افزار Arduino IDE</li>
      <li>کتابخانه <code>Servo.h</code></li>
      <li>درایورهای استپ موتور</li>
      <li>سیم‌کشی مطابق شماتیک</li>
    </ul>

    <h2>🚀 نحوه اجرا</h2>
    <ol>
      <li>نصب Arduino IDE</li>
      <li>بارگذاری فایل <code>main.ino</code></li>
      <li>انتخاب برد Arduino مناسب</li>
      <li>آپلود کد و اتصال مدار مطابق تصویر</li>
      <li>راه‌اندازی و تست عملکرد</li>
    </ol>

    <h2>🧠 توسعه‌دهنده</h2>
    <p>
      <strong>رضا احمدی</strong><br>
      دانشجوی کارشناسی ارشد مهندسی برق – گرایش الکترونیک دیجیتال<br>
      دانشگاه افرش<br>
      GitHub: <a href="https://github.com/rezahmadi" target="_blank">rezahmadi</a>
    </p>

    <div class="footer">
      © 2025 رضا احمدی – تمامی حقوق محفوظ است.
    </div>
  </div>
</body>
</html>

